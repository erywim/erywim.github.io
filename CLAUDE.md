...
**后台联调**：`PUBLIC_API_BASE_URL=http://127.0.0.1:8787 bun run dev`（worker 目录先 `bun run dev`；账号 `erywimond`，密码见 worker README 种子流程）。
...
| `/about` | 勇者档案（角色面板 + 技能 + 冒险历程） | `src/data/hero.json` |
| `/logs` | 旅行日志（周报） | `src/content/logs/` |
| `/links` | 伙伴酒馆（友链） | `src/data/friends.json` |
...
- **勇者档案** `src/data/hero.json`（类型在 `src/data/types.ts`）：名字/职业/属性/简介/技能/冒险历程。
- **伙伴酒馆** `src/data/friends.json`：友链列表（name/role/intro/link/avatar）。link 为 '#' 时渲染为不可点击。
